...
if __name__ == "__main__":
    # 按照标签提取切片，只保存有肝脏的切片
    # nii2png_single(r'nii_data\CT_labels\00001077755.nii',IsData=True)
    # 对nii_data\CT_labels的每一个nii文件，使用nii2png_single提取切片,并保存到png/single文件夹下
    import os
    nii_path = r'nii_data\CT_labels'
    nii_list = os.listdir(nii_path)
    for nii in nii_list:
        nii2png_single(os.path.join(nii_path, nii), IsData=True)
